...
function Cell({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <td className={`whitespace-nowrap px-4 py-3 text-sm text-zinc-300 ${className}`}>
      {children}
    </td>
  );
}
        ...
        <tbody>
          {specs.map((spec, idx) => {
            ...
            return (
              ...
              >
                <Cell className="font-semibold text-zinc-100">{spec.name}</Cell>
                ...
                <Cell>
                  <span className="text-zinc-400">{spec.useCase.join(" · ")}</span>
                </Cell>
              </tr>
            );
          })}
